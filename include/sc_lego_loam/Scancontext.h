...
public:
    ...
    // 进行kd搜索时选取最接近的10帧
    const int    NUM_CANDIDATES_FROM_TREE = 5; // 10 is enough. (refer the IROS 18 paper)
    ...
    const double SC_DIST_THRES = 0.0; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold
    ...
    std::vector<LidarIris::FeatureDesc> IrisFeatures_;
    ...
    // LidarIris iris(4, 18, 1.6, 0.75, 50); //error: expected identifier before numeric constant
    LidarIris LidarIris_ = LidarIris(4, 18, 1.6, 0.75, 50);
